...
def train_simple(batch_size, epochs, folds=5):
    ...
    for f in range(folds):
        ...
        train_data = DataLoader(dataset_train, batch_size=batch_size, num_workers=2, pin_memory_device='cuda:'+str(device), shuffle=True)
        val_data = DataLoader(dataset_val, batch_size=batch_size, num_workers=2, pin_memory_device='cuda:'+str(device), shuffle=True)
        ...
        train_cnn = Trainer(loss_fn=loss_function, optimizer=optimizer, model=model, scheduler=scheduler)
